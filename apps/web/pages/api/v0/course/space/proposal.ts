...
import { PROPOSAL_QUERY } from '@dae/snapshot'
...
const handleGetRequest = async (
  req: NextApiRequest,
  res: NextApiResponse<ApiResponse<PROPOSAL_QUERY>>,
) => {
  try {
    const { proposalId, chainId } = req.query as {
      proposalId: string
      chainId: string
    }
    ...
    const proposal = await client.request<PROPOSAL_QUERY>(PROPOSAL_QUERY, {
